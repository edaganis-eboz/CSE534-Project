...
class Client_Data_Plane():
    ...
    def receive_via_SA(self):
        def packet_handler(packet):
            if Ether in packet and Raw in packet:
                ...
                ciphertext = raw_payload[sectag_size:-16]
                print("ciphertext received: ", ciphertext)
                icv = raw_payload[-16:]

                sectag_deserialized = self.deserialized_sectag(sectag)

                data = self.decrypt_data(ciphertext, sectag_deserialized['sa_ID'])
    ...
    def deserialized_sectag(self, sectag):
        ...
        return plain_sectag
    def decrypt_data(self, ciphertext, sa_ID):
        ...
        plaintext = SA.cipher.decrypt(ciphertext)
